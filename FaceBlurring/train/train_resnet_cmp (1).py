...
def visualize(model, input_size, device):
    ...
    with torch.no_grad():
        ...
        plt.legend(fontsize=15)
        plt.savefig(f"graph_complex_visualize.png")
...
if __name__ == '__main__':
    ...
    os.makedirs("./checkpoint/resnet18_complex/", exist_ok=True)
    ...
    for epoch in range(epochs):
        ...
        scheduler.step()
        torch.save(model, f"./checkpoint/resnet18_complex/checkpoint_{epoch}.pt")
        model.eval()
        with torch.no_grad():
            validation_loss = 0.0
    ...
    plt.legend(fontsize=15)
    plt.savefig("resnet18_complex_loss.png")
