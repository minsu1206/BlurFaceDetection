...
class CreateBlurImages:
    ...
    def generate_blur_kernels(self, save=True, num_filters=100000):
        ...
        # Save option
        if save:
            filters['method'] = np.array(filters['method'])
            with open("filters.pkl", 'wb') as f:
    ...
    def generate_blur_images(self, scrfd=False, num_samples=1, batch=1):
        ...
        '''
        # Filters
        self.filters['method'], self.filters['kernel'] = np.array(self.filters['method']), np.array(self.filters['kernel'], dtype=object)
        if self.blur_method != 'random':
            idxs = np.where(self.filters['method']==self.blur_method)
            self.filters['kernel'] = self.filters['kernel'][idxs]
